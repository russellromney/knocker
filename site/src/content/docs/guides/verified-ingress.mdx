...
provider verification lives in the SQLite/Rust layer, while Python also
supports app-local provider instances.

## Provider tiers

For Python app-local providers, Knocker exposes a small public interface
(`Provider`, `ProviderRequest`, `ProviderResult`) plus a per-`Knocker`
registry. Across the product, provider support has three tiers:
...
built-ins. In Python, app-local and community providers pass a `Provider`
instance to `add_endpoint(...)`; an instance whose `.name` collides with a
curated name is rejected. There is no global string registry for
non-curated providers, so a future curated addition cannot collide with your
custom provider.
...
Those fixtures are exercised by Rust core tests and by the Python provider
mirror so the documented behavior stays tied to executable examples.
