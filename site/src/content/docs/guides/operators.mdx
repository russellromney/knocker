...
```text
list_events(status="failed", endpoint="stripe", event_type="checkout.session.completed", since=1700000000, limit=50)
```
...
```text
list_deliveries(endpoint="stripe", signature_valid=false, orphaned=true, since=1700000000, limit=50)
```
...
get_event(42)
get_delivery(99)
list_deliveries(event_id=42, limit=50)
```
