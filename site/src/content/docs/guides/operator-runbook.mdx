...
These numbers assume the ordinary same-process shape: one long-lived
`knocker.open(...)` per process, reused for ingress and local workers.
Opening many independent Knocker handles against one SQLite file is
supported, but it is intentionally treated as a degraded contention mode.
...
```python
audits = app.list_prune_audits(limit=20)
for audit in audits:
