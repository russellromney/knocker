...
Knocker is a loadable SQLite extension plus language bindings for Python, Node, Bun, Ruby, Go, and Elixir. It is for apps that already have an HTTP server, a SQLite database, and business logic that wants to react to inbound webhook events without adding a hosted control plane.
