---
title: Knocker
description: Python-first inbound webhook inbox on SQLite.
template: splash
hero:
...
Knocker is a Python-first inbound webhook inbox for apps that already have an HTTP server, a SQLite database, and business logic that wants to react to inbound events without adding a hosted control plane.

> Pre-0.1.0. API may change; until the first release is cut, install from this repository rather than PyPI.
...
- Verify requests with Stripe or generic HMAC-SHA256, including active secret rotation
