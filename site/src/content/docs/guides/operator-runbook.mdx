...
Start by listing recent events and deliveries for the endpoint you care
about: recent events, invalid deliveries, and orphan deliveries. Use the
[Operator surface](/guides/operators/) page for binding-specific calls.
...
Ignore events that are still safe to skip. `ignore(...)` is accepted from
...
If `run_worker(...)` raises outside normal handler retry/dead-letter
...
Pruning is explicit and irreversible. Preview candidates with read APIs
first, then prune with a bounded threshold. Prune old orphan deliveries
separately. Every prune call writes a durable audit row. Use the
[Retention and pruning](/guides/retention/) page for binding-specific calls.
